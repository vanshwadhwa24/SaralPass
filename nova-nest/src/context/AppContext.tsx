...
// Helper function to determine status based on train ETA
const determineStatusFromETA = (trainETA: string): CrossingData["status"] => {
  // If no data (N/A, empty, or invalid), return maintenance
  if (!trainETA || trainETA === "N/A" || trainETA.trim() === "") {
    return "maintenance";
  }

  // Extract minutes from train ETA
  const etaLower = trainETA.toLowerCase();
  let minutes = 0;

  if (etaLower.includes("min")) {
    // Extract number before "min"
    const match = etaLower.match(/(\d+(?:\.\d+)?)\s*min/);
    if (match) {
      minutes = parseFloat(match[1]);
    }
  } else if (etaLower.includes("hr") || etaLower.includes("hour")) {
    // Convert hours to minutes
    const match = etaLower.match(/(\d+(?:\.\d+)?)\s*(?:hr|hour)/);
    if (match) {
      minutes = parseFloat(match[1]) * 60;
    }
  } else {
    // Try to extract any number
    const match = etaLower.match(/(\d+(?:\.\d+)?)/);
    if (match) {
      minutes = parseFloat(match[1]);
    }
  }

  // Apply new logic: < 10 min = active, >= 10 min = clear
  return minutes < 10 ? "active" : "clear";
};
  ...
  crossings: [
    {
      ...
      status: determineStatusFromETA("2 min"),
      trainETA: "2 min",
      rerouteActive: true,
      avgDelay: "4.2 min",
    ...
    },
    {
      ...
      status: determineStatusFromETA("N/A"),
      trainETA: "N/A",
      rerouteActive: true,
      avgDelay: "N/A",
      lastUpdated: new Date(),
    ...
    },
    {
      ...
      status: determineStatusFromETA("45 min"),
      trainETA: "45 min",
      rerouteActive: false,
      avgDelay: "0.8 min",
      lastUpdated: new Date(),
    ...
    },
    {
      id: 4,
      name: "Commerce Way",
      status: determineStatusFromETA("1.2 hr"),
      trainETA: "1.2 hr",
      rerouteActive: false,
      avgDelay: "1.1 min",
      lastUpdated: new Date(),
    ...
    },
    {
      ...
      status: determineStatusFromETA("8 min"),
      trainETA: "8 min",
      rerouteActive: true,
      avgDelay: "3.1 min",
      lastUpdated: new Date(),
...
function appReducer(state: AppState, action: AppAction): AppState {
  switch (action.type) {
    ...
    case "REFRESH_DATA":
      // Simulate data refresh and update status based on train ETA
      return {
        ...state,
        crossings: state.crossings.map((crossing) => ({
          ...crossing,
          status: determineStatusFromETA(crossing.trainETA),
          lastUpdated: new Date(),
        })),
      };
